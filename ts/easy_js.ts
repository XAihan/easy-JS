...
class easy_js {
    ...
    static base: any = {
        ...
        getTime(times?: number, format?: string) {
            let time = times ? new Date(parseInt(times.toString().padEnd(13, '0'))) : new Date(),
        ...
        transferParam(){
            $('a').on('click',()=>{
                ...
                href = href.split('#');
                if (href.length > 1) src = href[0] + window.location.search + '#' + href[1]; else src = href[0] + window.location.search;
                $(this).attr('href', src);
            })
        }
    ...
    static event:any = {
        ...
        scroll(height:any,doneCallback:Function,doCallback?:Function){
            $(window).scroll(()=>{
                let s = $(document).scrollTop(); //获取滚动高度
                let top = (typeof height === 'number')?height: $(height).offset().top
                if(s>top){
                    doneCallback()
                }else{
                    doCallback()
